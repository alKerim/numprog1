public class Gleitpunktzahl {
    ...
    private static int highestExponent = (int) (Math.pow(2, sizeExponent) / 2) -1; // = expoffset
    ...
    public static void denormalisiere(Gleitpunktzahl a, Gleitpunktzahl b) {
         ...
         */
        if (a.exponent==b.exponent || a.compareAbsTo(b)==0)
            return;
    ...
    public Gleitpunktzahl add(Gleitpunktzahl r) {
        ...
        // this> r und this >0 -> positif
        else if (this.compareAbsTo(r)>= 1 && !this.vorzeichen && r.vorzeichen) {
            result.mantisse = this.mantisse - r.mantisse;
            result.vorzeichen= false;
        }
        // this <r und r>0 -> postitf
        else if (this.compareAbsTo(r)<-1 && this.vorzeichen && !r.vorzeichen){
        ...
        // this > r und this <0
            ...
            result.mantisse = this.mantisse - r.mantisse;
            result.vorzeichen= true; // false = positif
        }
        // this< r und r<0
        else if (this.compareAbsTo(r)<-1 && !this.vorzeichen && r.vorzeichen){
            result.mantisse = r.mantisse - this.mantisse;
      ...
      // sonderfälle: ergebnis out of bound
